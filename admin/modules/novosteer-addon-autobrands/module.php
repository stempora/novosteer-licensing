<?php
/*
	Stempora Web Framework
	Copyright (c) 2002-2020 Stempora. 
	All rights reserved.
		web:  www.stempora.com
		mail: support@stempora.com				
*/

if (!defined("STPBase")) {
	die("This file can't be accessed directly!");
}


return [
	"tables"	=> [
		"plugin:novosteer_addon_autobrands_brands"						=> "site_plugin_novosteer_addon_autobrands_brands",
		"plugin:novosteer_addon_autobrands_models"						=> "site_plugin_novosteer_addon_autobrands_models",
		"plugin:novosteer_addon_autobrands_trims"						=> "site_plugin_novosteer_addon_autobrands_trims",

		"plugin:novosteer_addon_autobrands_types"						=> "site_plugin_novosteer_addon_autobrands_types",

	]
];		
		
