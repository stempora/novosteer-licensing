...
class DealerImages extends Importer implements ImporterInterface{
	...
	*/
	function updateProduct(&$product , &$data , &$item) {
		...
		$this->log("Calculating price for %s" , [$data["vin"]]);

		$vehicle = $this->getVehicleByID($data["product_id"]);
		$this->calculator->setVehicle($vehicle);
		$price = $this->calculator->calculatePrice();

		if ($price === false) {
			$this->log("No rule matched for %s" , [$data["vin"]]);
		} else {
		...
		$this->db->QueryUpdateByID(
			$this->module->tables["plugin:novosteer_vehicles"],
			[
				$this->info["settings"]["set_price_field"] => $price !== false ? $price : -1,
			],
			$vehicle["product_id"]
		);
